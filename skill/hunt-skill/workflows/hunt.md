...
- **Execution-context identity closure:** invocation mode -> call/delegatecall/forward/callback edge -> effective `msg.sender`, `address(this)`, and storage owner -> downstream authorization or accounting -> sensitive effect. Trigger when shared helpers or implementations are directly callable, a contract forwards caller-selected execution, or a downstream system trusts a contract address as owner, delegate, payer, pool, or other authority. Compare the intended proxy/delegate/callback route with direct and forwarded routes, and graph every place the effective contract identity can be lent or accepted.
...
4. Build or update the `JOB_ATTACK_MODEL`, then use selected dimensions together around this same forbidden state:
   - local code/dataflow: origin, cache, normalization, precision domain, rounding direction and beneficiary, writers, consumers, sibling paths, guards, callbacks, inverse/cancel paths;
