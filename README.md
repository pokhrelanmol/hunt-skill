...
[Risk classification](skill/hunt-skill/references/risk-mapping.md) organizes important unresolved assumptions within that map. Concerns may carry labels such as precision, pricing, integration, lifecycle, or availability, and may span several components. Labels help retrieval; evidence and the value of the next check determine the Job shortlist. Unclassified concerns remain eligible. Existing facts preserve the question, supporting/counterevidence, and what was actually reviewed; no extra database or mandatory classification pass is required.
...
State Probes deliberately poke underexplored reachable behavior: `1 wei`, dust, threshold +/- 1, partial operations, repeated operations, equivalent paths such as `deposit(100)` versus `deposit(40); deposit(60)`, economic inverse paths such as mint/redeem or exact-in/exact-out, operation reordering, different actors, time boundaries, and realistic external-state changes. A caller-favorable rounding delta is tested for splitting, repetition, reset, accumulated state drift, nonlinear boundary crossing, downstream consumption, and full-cycle economics before it is dismissed as harmless dust. Unexpected results become `OBSERVATION`, not automatically a bug.
